""" A module for Bayesian parameter estimation of an exponential distribution with trend in the mean. """
...
class TrendExponentialModel(ClimTrendModel):
    ...
    def get_percentile_of_percentile_at_time(self,
        ...
        if self.sampler is not None:
            ...
            model_values = exponential_ppf_fast(model_percentile, mu)
    ...
    def generate_samples(self,
        ...
        samples = np.reshape(np.array( [ scipy.stats.expon.rvs(scale = 1/m) for m in mu ] ), 
                             np.shape(dates))
